use anchor_lang::prelude::*;
use anchor_spl::token::{burn, transfer, Burn, Mint, Token, TokenAccount, Transfer};

use crate::{Market, Outcome, PredictionMarketError};

#[derive(Accounts)]
pub struct ClaimAmmWinnings<'info> {
    #[account(mut)]
    pub user: Signer<'info>,

    #[account(
        seeds = [
            b"market",
            market.authority.as_ref(),
            &market.market_id.to_le_bytes()
        ],
        bump = market.bump
    )]
    pub market: Box<Account<'info, Market>>,

    /// CHECK: PDA used only as the authority of the AMM payment vault.
    #[account(
        seeds = [
            b"amm-authority",
            market.key().as_ref()
        ],
        bump
    )]
    pub amm_authority: UncheckedAccount<'info>,

    #[account(
        mut,
        seeds = [
            b"payment-vault",
            market.key().as_ref()
        ],
        bump,
        constraint = payment_vault.mint == market.payment_mint
            @ PredictionMarketError::InvalidMint,
        constraint = payment_vault.owner == amm_authority.key()
            @ PredictionMarketError::Unauthorized
    )]
    pub payment_vault: Box<Account<'info, TokenAccount>>,

    #[account(
        mut,
        seeds = [
            b"yes-mint",
            market.key().as_ref()
        ],
        bump
    )]
    pub yes_mint: Box<Account<'info, Mint>>,

    #[account(
        mut,
        seeds = [
            b"no-mint",
            market.key().as_ref()
        ],
        bump
    )]
    pub no_mint: Box<Account<'info, Mint>>,

    #[account(
        mut,
        constraint = user_yes_account.owner == user.key()
            @ PredictionMarketError::Unauthorized,
        constraint = user_yes_account.mint == yes_mint.key()
            @ PredictionMarketError::InvalidMint
    )]
    pub user_yes_account: Box<Account<'info, TokenAccount>>,

    #[account(
        mut,
        constraint = user_no_account.owner == user.key()
            @ PredictionMarketError::Unauthorized,
        constraint = user_no_account.mint == no_mint.key()
            @ PredictionMarketError::InvalidMint
    )]
    pub user_no_account: Box<Account<'info, TokenAccount>>,

    #[account(
        mut,
        constraint = user_payment_account.owner == user.key()
            @ PredictionMarketError::Unauthorized,
        constraint = user_payment_account.mint == market.payment_mint
            @ PredictionMarketError::InvalidMint
    )]
    pub user_payment_account: Box<Account<'info, TokenAccount>>,

    pub token_program: Program<'info, Token>,
}

pub fn claim_amm_winnings(ctx: Context<ClaimAmmWinnings>) -> Result<()> {
    let market = &ctx.accounts.market;

    require!(
        market.outcome == Outcome::Yes || market.outcome == Outcome::No || market.outcome == Outcome::Cancelled,
        PredictionMarketError::MarketNotResolved
    );

    let amount_yes = ctx.accounts.user_yes_account.amount;
    let amount_no = ctx.accounts.user_no_account.amount;

    let payout_amount = match market.outcome {
        Outcome::Yes => {
            require!(amount_yes > 0, PredictionMarketError::NoWinningShares);
            
            let yes_burn_accounts = Burn {
                mint: ctx.accounts.yes_mint.to_account_info(),
                from: ctx.accounts.user_yes_account.to_account_info(),
                authority: ctx.accounts.user.to_account_info(),
            };
            burn(CpiContext::new(ctx.accounts.token_program.key(), yes_burn_accounts), amount_yes)?;
            
            amount_yes
        }
        Outcome::No => {
            require!(amount_no > 0, PredictionMarketError::NoWinningShares);
            let no_burn_accounts = Burn {
                mint: ctx.accounts.no_mint.to_account_info(),
                from: ctx.accounts.user_no_account.to_account_info(),
                authority: ctx.accounts.user.to_account_info(),
            };
            burn(CpiContext::new(ctx.accounts.token_program.key(), no_burn_accounts), amount_no)?;
            
            amount_no
        }
        Outcome::Cancelled => {
            require!(amount_yes > 0 || amount_no > 0, PredictionMarketError::NoWinningShares);
            let mut total_payout = 0;
            if amount_yes > 0 {
                let yes_burn = Burn {
                    mint: ctx.accounts.yes_mint.to_account_info(),
                    from: ctx.accounts.user_yes_account.to_account_info(),
                    authority: ctx.accounts.user.to_account_info(),
                };
                burn(CpiContext::new(ctx.accounts.token_program.key(), yes_burn), amount_yes)?;
                total_payout += amount_yes / 2;
            }
            if amount_no > 0 {
                let no_burn = Burn {
                    mint: ctx.accounts.no_mint.to_account_info(),
                    from: ctx.accounts.user_no_account.to_account_info(),
                    authority: ctx.accounts.user.to_account_info(),
                };
                burn(CpiContext::new(ctx.accounts.token_program.key(), no_burn), amount_no)?;
                total_payout += amount_no / 2;
            }
            total_payout
        }
        _ => return err!(PredictionMarketError::MarketNotResolved),
    };

    if payout_amount > 0 {
        let market_key = market.key();
        let amm_bump = [ctx.bumps.amm_authority];
        let amm_signer_seeds: &[&[u8]] = &[b"amm-authority", market_key.as_ref(), &amm_bump];
        let signer_seeds = &[amm_signer_seeds];

        require!(
            ctx.accounts.payment_vault.amount >= payout_amount,
            PredictionMarketError::InsufficientVaultFunds
        );

        let transfer_accounts = Transfer {
            from: ctx.accounts.payment_vault.to_account_info(),
            to: ctx.accounts.user_payment_account.to_account_info(),
            authority: ctx.accounts.amm_authority.to_account_info(),
        };

        let transfer_ctx = CpiContext::new_with_signer(
            ctx.accounts.token_program.key(),
            transfer_accounts,
            signer_seeds,
        );
        transfer(transfer_ctx, payout_amount)?;
    }

    Ok(())
}
